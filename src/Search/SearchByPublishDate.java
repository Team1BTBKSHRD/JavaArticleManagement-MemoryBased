...
public class SearchByPublishDate implements ISearch {
	...
	public ArrayList<Integer> search(ArrayList<Article> articles, String key) {
		...
		for (int startIndex = 0, halfIndex = articles.size() / 2; startIndex < articles
			...
			if (articles.get(startIndex).getPublishDate().toLowerCase()
					.startsWith(key)) {
				indices.add(startIndex);
			}
			if (articles.get(halfIndex).getPublishDate().toLowerCase()
					.startsWith(key)) {
		...
		if ((articles.size() % 2) != 0) {
			if (articles.get(articles.size() - 1).getPublishDate()
